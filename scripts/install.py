...
if __name__ == "__main__":
    ...
    for pkg_name in pkg_name_list:
        ...
        if pkg.is_installed:
            ...
            try:
                ...
                print("[i] {pkg_name} installed successfully".format(pkg_name=pkg_name))

    pip_list = ["twilio", "qrcode", "Adafruit-DHT", "gtts", "phonenumbers"]
    for pip_pkg in pip_list:
        print("[I] Installing {} using pip...".format(pip_pkg))
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", pip_pkg], stdout=subprocess.DEVNULL)
        except Exception as e:
            sys.exit("[E] Sorry, package installation of {} failed [{}]".format(pip_pkg, str(e)))
        else:
    ...
    try:
        subprocess.check_call(["git", "clone", "-b", "5.15", "https://github.com/qt/qtvirtualkeyboard.git"], stdout = subprocess.DEVNULL)
    except Exception as e:
    ...
    try:
        subprocess.check_call(["cp", "{}/lib/libQt5VirtualKeyboard.so.5.15.8".format(new_path), "{}/lib/libQt5VirtualKeyboard.so.5".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["mkdir", "{}/plugins".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["mkdir", "{}/plugins/platforminputcontexts".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["cp", "{}/plugins/platforminputcontexts/libqtvirtualkeyboardplugin.so".format(new_path), "{}/plugins/platforminputcontexts/".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["cp", "-r", "{}/plugins/virtualkeyboard/".format(new_path), "{}/plugins/".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["mkdir", "{}/qml".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["mkdir", "{}/qml/QtQuick".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
        subprocess.check_call(["cp", "-r", "{}/qml/QtQuick/VirtualKeyboard/".format(new_path), "{}/qml/QtQuick/".format(QT_PREFIX_PATH)], stdout = subprocess.DEVNULL)
    except Exception as e:
